...
class SparsePolynomial : public sparse_vector_t {
 public:
  ...
  // Modificacion
  double AverageOfOddDegreeCoeficients() const; 
};
...
double SparsePolynomial::AverageOfOddDegreeCoeficients() const {
  double suma{0.0};
  double contador{0.0};
  for(int i = 0; i < get_nz(); i++) {
    if(at(i).get_inx() % 2 != 0) {
...
//bool SparsePolynomial::exponentesConsecutivos() const {
//  if (get_nz() <= 1) {
//    return true;
//  }
//  for (int i = 1; i < get_nz(); i++) {
//    if (at(i).get_inx() != at(i - 1).get_inx() + 1) {
//     return false;
//    }
//  }
//  return true;
