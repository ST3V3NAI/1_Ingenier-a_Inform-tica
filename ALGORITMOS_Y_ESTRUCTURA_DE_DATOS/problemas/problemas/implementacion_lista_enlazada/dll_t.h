...
template <class T> class dll_t {
 public:
  ...
  // Signature of insert_after
  void insert_after(dll_node_t<T>* ref_node, dll_node_t<T>* new_nodo);
...
template <class T>  
void dll_t<T>::insert_after(dll_node_t<T>* ref_node, dll_node_t<T>* new_nodo) {
  // Check wether the list is empty
  assert(!empty());
  // Check that node is not pointed to NULL
  assert(ref_node != NULL && new_nodo != NULL);

  // Check wether ref_node is the last node
  if(ref_node == tail_) { // if(ref_node == this->tail_) // (ref_node->get_next() != NULL=
      push_back(new_nodo);
  } else {
    new_nodo-> set_prev(ref_node);
    new_nodo-> set_next(ref_node->get_next());
    ref_node-> set_next(new_nodo);
    new_nodo -> get_next()->set_prev(new_nodo);
    sz_++; // Updtae the length of the list 
  }
}
